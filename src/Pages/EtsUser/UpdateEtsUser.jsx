...
const UpdateEtsUser = () => {
    ...
    try {
      ...
      if (res?.status) {
        ...
        setFormData({
          ...
          mobile: user.mobile || "",
          gender: user.gender || "",
          profilePic: "", // keep empty for new upload
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await updateEtsUserApi({
        id,
        data: formData,
      });

      if (res?.status) {
        toast.success("ETS User updated successfully!");
        navigate(-1);
      }
    } catch (err) {
      ...
      <div className="mt-8 bg-white p-6 w-full rounded-xl shadow-xl">
        <form onSubmit={handleSubmit}>
          ...
          <label
            ...
            className="flex items-center justify-center h-10 border border-gray-500 rounded-xl cursor-pointer bg-white hover:bg-gray-100"
          >
