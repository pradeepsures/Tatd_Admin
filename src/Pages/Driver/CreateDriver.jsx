...
const CreateDriver = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const prefs = await getAllVehiclePreferences({});
        const cats = await getAllVehiclePreferenceCategories({});

        setVehiclePrefs(prefs?.data || []);
        setVehicleCategories(cats?.data || []);
      } catch (err) {
        console.log(err);
      }
    };
    ...
    <div className="m-3">
      ...
      <div className="ml-5 mt-8 bg-white p-6 max-w-9xl rounded-xl shadow-xl">
        <form onSubmit={handleSubmit}>
          ...
          <label className="ml-2 mt-6 font-normal block">
            Vehicle Preferences
          </label>
