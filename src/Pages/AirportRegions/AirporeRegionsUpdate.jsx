...
import {
  getSingleAirportRegion,
  updateAirportRegion,
} from "../../Services/AirportRegionApi";

const UpdateAirportRegion = () => {
  ...
  // ================= CLEAR =================
  const handleClear = () => {
    fetchRegion(); // reset to original data
    setApiError({});
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updateAirportRegion(id, payload);

      if (res?.status) {
        toast.success("Airport Region Updated");
        navigate(-1);
      } else {
...
export default UpdateAirportRegion;