...
import {
  getAllWeeklyBookings,
  assignWeeklyDriver,
  reassignWeeklyDriver,
  getUnassignedWeeklyDrivers,
} from "../../Services/BookingApi";

const { Option } = Select;

export default function WeeklyBookingList() {
  ...
  const fetchBookings = useCallback(async () => {
    try {
      setLoading(true);
      const res = await getAllWeeklyBookings({
        page,
        limit: rowsPerPage,
        ...filters,
      });
      ...
      toast.error("Error fetching weekly bookings");
    } finally {
      setLoading(false);
    }
  ...
  const fetchDrivers = useCallback(async () => {
    ...
    try {
      const res = await getUnassignedWeeklyDrivers(selectedRowId, searchDriver);
      if (res?.status) {
        setDrivers(res.data || []);
      }
    } catch {
  ...
  const handleAssignSubmit = async () => {
    ...
    try {
      ...
      if (isReassignModalOpen) {
        res = await reassignWeeklyDriver(selectedRowId, selectedDriver);
      } else {
        res = await assignWeeklyDriver(selectedRowId, selectedDriver);
      }
  ...
  const exportExcel = () => {
    ...
    try {
      xlsx(exportData, { fileName: "Weekly_Bookings_List" });
    } finally {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-4">
        <h1 className="text-xl font-bold text-gray-900 font-sans">Weekly Bookings</h1>
        <p className="text-sm text-gray-500 mt-1">Manage and assign chauffeurs to weekly plans.</p>
      </div>
                ...
                <TableRow key={row._id}>
                  ...
                  <TableCell align="center">
                    ...
                    <Menu anchorEl={anchorEl} open={Boolean(anchorEl) && selectedRowId === row._id} onClose={handleMenuClose}>
                      <MenuItem onClick={() => navigate(`/home/booking/bookingdetails/${row._id}?type=weekly`)}>
