...
import { getShuttleRouteById } from "../../Services/ShuttleRouteApi"; // ✅ your fetch function

const ShuttleRouteDetails = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      try {
        ...
        if (res?.status) {
          setRoute(res.data); // ✅ store full object
        }
      } catch (err) {
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white p-6 rounded-xl shadow mb-6">
        <h2 className="text-2xl mb-4">
          <strong>Shuttle Route Name:</strong> {route.name || "N/A"}
        </h2>
