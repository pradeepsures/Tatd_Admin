...
import {
  getShuttleRouteById,
  updateShuttleRouteApi,
} from "../../Services/ShuttleRouteApi";

const UpdateShuttleRoute = () => {
  ...
  // ✅ FETCH ROUTE DATA TO PREFILL FORM
  useEffect(() => {
    const fetchRoute = async () => {
      try {
        ...
        toast.error("Failed to load shuttle route");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.stoppages || formData.stoppages.length < 2) {
      return toast.error("At least 2 stoppages are required");
    }

    setLoading(true);
    try {
      const res = await updateShuttleRouteApi({ id, data: formData });
      if (res?.status) {
        toast.success("Shuttle Route updated successfully");
        navigate(-1);
      }
    } catch (err) {
...
export default UpdateShuttleRoute;