...
import {
  getShuttlePassById,
  updateShuttlePassApi,
} from "../../Services/ShuttlePassApi";

const UpdateShuttlePass = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (res?.status) {
          ...
          if (data.thumbImage) {
            setImagePreview(data.thumbImage);
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updateShuttlePassApi({ id, data: fd });

      if (res?.status) {
        toast.success("Shuttle Pass updated successfully");
        navigate(-1);
      }
    } catch (err) {
      ...
      <div className="mt-6 bg-white p-6 rounded-xl shadow-xl w-full">
        <form onSubmit={handleSubmit}>
          ...
          <label
            ...
            className="flex items-center justify-center w-full h-12 border border-gray-400 rounded-xl cursor-pointer bg-white hover:bg-gray-100"
          >
            🖼️ Upload Thumbnail
          </label>
...
export default UpdateShuttlePass;